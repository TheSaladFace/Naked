...
jQuery('.post-slider-2-buttons').css({ opacity: 1 });
});
...
jQuery(document).ready(function(){
	...
	**/
	...
	jQuery('.equal-heights-slider').matchHeight({
        target: jQuery('.naked-featured-slider-2')
    });
	...
	*/
	function updateMinimalArrows(arrow,sliderIdentifier)
	{
		...
		/** Remove duplicate descriptions,  users can add multiple titles and titles + excerpt-string
		    This removes the second instance twice, just in case the user adds 3 titles / titles+ excerpt **/
		jQuery('.featured-slider-2 .next-desc-clone').eq(1).remove();
		jQuery('.featured-slider-2 .prev-desc-clone').eq(1).remove();
		jQuery('.featured-slider-2 .next-desc-clone').eq(1).remove();
		jQuery('.featured-slider-2 .prev-desc-clone').eq(1).remove();
	}

	jQuery(".featured-post-slider-2-next").each( function() {
	...
	jQuery(".featured-post-slider-2-prev").each(function() {
		var arrow=jQuery(this).parent();
		var sliderContainer=jQuery(this).closest('.featured-thumbnail-slider');
	...
	jQuery(".naked-featured-slider-2").each( function() {
	...
	//Initial setup
	jQuery( ".featured-post-slider-2-next" ).click(function() {
			var arrow=jQuery(this).parent();
	...
	jQuery( ".featured-post-slider-2-prev" ).click(function() {
			var arrow=jQuery(this).parent();
			var sliderContainer=jQuery(this).parent().parent();
			sliderContainer.find(".slick-slider").slick('slickPrev');
			updateMinimalArrows(arrow,sliderContainer);
