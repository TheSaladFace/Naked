...
if ( ! empty( $atts['background_parallax_ratio'] ) ) {
	wp_enqueue_script('naked-stellar', $uri . '/static/js/jquery.stellar.min.js',array('jquery'),'',true ); //will enqueue from the default theme location but will load from here if theme is changed
	wp_enqueue_script('naked-stellar-init', $uri . '/static/js/stellar-init.js',array('jquery'),'',true );
